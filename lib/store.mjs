...
export class Store {
  constructor(directory) {
    ...
    this.AuditLog = this.sequelize.define("AuditLog", {
      ...
      hostPath: { type: DataTypes.TEXT, allowNull: false },
      remoteAddress: { type: DataTypes.STRING, allowNull: false },
      outcome: { type: DataTypes.STRING, allowNull: false },
      details: { type: DataTypes.TEXT, allowNull: false },
    }, { timestamps: false, indexes: [{ fields: ["protocol", "id"] }, { fields: ["outcome", "id"] }] });
    this.auditBuffer = [];
    this.ready = (async () => {
  ...
  audit(entry) {
    ...
    for (const key of ["protocol", "user", "action", "path", "destination", "hostPath", "remoteAddress", "outcome", "details"])
      record[key] = String(entry[key] ?? "").slice(0, key === "details" ? 2000 : 4096);
    if (entry.timestamp && Number.isFinite(Date.parse(entry.timestamp)))
      record.timestamp = new Date(entry.timestamp).toISOString();
    this.auditBuffer.push(record);
    this.auditTimer ||= setTimeout(() => { void this.flushAudit(); }, 100);
  ...
  flushAudit() {
    ...
    if (!this.auditBuffer.length) return this.queue;
    const entries = this.auditBuffer.splice(0);
    const result = this.queue.then(async () => {
      await this.ready;
      await this.AuditLog.bulkCreate(entries);
      // Keep a bounded history without deleting newer events as the log grows.
      await this.sequelize.query('DELETE FROM "AuditLogs" WHERE "id" <= (SELECT MAX("id") - 100000 FROM "AuditLogs")');
      this.auditError = null;
    });
    this.queue = result.catch(error => {
      this.auditError = "Some activity could not be saved. Check available disk space and server logs.";
  ...
  clearAudit() {
    const result = this.flushAudit().then(async () => {
      await this.ready;
      await this.AuditLog.destroy({ where: {} });
    });
