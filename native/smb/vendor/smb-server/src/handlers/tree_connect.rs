...
pub async fn handle(
    ...
    // Anonymous discovery is permitted only when an anonymous share exists.
    let share = if share_name.eq_ignore_ascii_case("IPC$") {
        if matches!(identity, Identity::Anonymous) && !server.anonymous_allowed().await {
    ...
    drop(acl);
    // Backend cap.
    let granted = if !share.is_ipc && share.backend.capabilities().is_read_only {
...
pub(super) fn authorize(
    ...
    match mode {
        ...
        ShareMode::Anonymous(access) => match identity {
            Identity::Anonymous => Some(*access),
