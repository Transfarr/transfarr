...
pub enum ShareMode {
    ...
    AuthenticatedOnly,
    /// Anonymous access alongside independent grants for named users.
    Anonymous(Access),
}
...
pub struct ShareBindings {
    ...
    /// `IPC$` synthetic share for SRVSVC discovery filtered by session permissions.
    pub is_ipc: bool,
}
...
impl ShareBindings {
    ...
    /// Synthetic IPC$ share. Pipe operations never reach the filesystem backend.
    pub fn ipc() -> Arc<Self> {
        Self::new(
            "IPC$".to_string(),
...
impl ServerState {
    ...
    pub async fn anonymous_allowed(&self) -> bool {
        for share in self.shares.all().await {
            ...
            if matches!(acl.mode, ShareMode::Public | ShareMode::PublicReadOnly | ShareMode::Anonymous(_)) {
                return true;
            }
        }
        false
...
impl ConfigHandle {
    /// TCP clients currently attached to this server, including discovery pipes.
    pub async fn connection_count(&self) -> usize {
        self.state.active_connections.live().await.len()
    }
